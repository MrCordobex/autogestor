"""
app2.py — AutoGestor v2: UI Streamlit optimizada.

Mejoras vs app.py:
  • @st.cache_data en cargas de DB (invalidación por versión en session_state).
  • Índice {fecha_iso: [items]} construido UNA vez por render → vistas O(1) por día.
  • Filtrado automático de tareas/eventos con fecha anterior a hoy
    (incluye limpieza en DB de tareas vencidas, no solo completadas).
  • Menos st.rerun() innecesarios; mutaciones invalidan el cache vía bump de versión.
  • Menos HTML/markdown por celda en vistas semanal/mensual.
"""

from __future__ import annotations

import calendar
import json
from collections import defaultdict
from datetime import date, datetime, timedelta, time

import pytz
import streamlit as st

import db
import scraper

# ─────────────────────────── CONFIG ────────────────────────────────────────

st.set_page_config(page_title="AutoGestor", layout="wide",
                   initial_sidebar_state="expanded")

TIMEZONE   = pytz.timezone("Europe/Madrid")
TIPOS_TAREA = ["Examen", "Entrega", "Estudio", "Lectura", "Otro"]
DIAS_ABR    = ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"]
DIAS_ES     = ["Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"]
MESES_ES    = ["Enero","Febrero","Marzo","Abril","Mayo","Junio",
               "Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"]
MESES_ABR   = ["ene","feb","mar","abr","may","jun",
               "jul","ago","sep","oct","nov","dic"]

COLORES_TIPO = {
    "Examen": "#FF4B4B", "Entrega": "#FFA500", "Estudio": "#1E90FF",
    "Lectura": "#9370DB", "Otro": "#808080",
    "Clase":  "#2E8B57", "Futbol": "#FFFFFF",
}
PRIO_ORDEN  = {"Urgente": 0, "Importante": 1, "Normal": 2}
PRIO_COLOR  = {"Urgente": "#FF4B4B", "Importante": "#FFA500", "Normal": "#2E8B57"}

CACHE_HORAS_SCRAPING = 12


# ─────────────────────────── HELPERS BÁSICOS ───────────────────────────────

def ahora_madrid() -> datetime: return datetime.now(TIMEZONE)
def hoy_madrid()  -> date:      return ahora_madrid().date()
def fstr(d: date) -> str:       return d.isoformat()

def fmt_dia_largo(d: date) -> str:
    """Ej: 'Jueves 07 may'."""
    return f"{DIAS_ES[d.weekday()]} {d.day:02d} {MESES_ABR[d.month-1]}"

def fmt_dia_corto(d: date) -> str:
    """Ej: '07 may 2026'."""
    return f"{d.day:02d} {MESES_ABR[d.month-1]} {d.year}"

def notificar(tipo: str, texto: str):
    st.session_state["_notif"] = (tipo, texto)

def mostrar_notificacion():
    n = st.session_state.pop("_notif", None)
    if n:
        (st.success if n[0] == "exito" else st.error)(n[1])

def bump_data():
    """Invalida los cachés de datos tras una mutación."""
    st.session_state["_data_v"] = st.session_state.get("_data_v", 0) + 1


# ─────────────────────────── CARGA DE DATOS (CACHED) ───────────────────────

@st.cache_data(show_spinner=False)
def _load_tareas(_v: int) -> list[dict]:
    return db.tareas_listar()

@st.cache_data(show_spinner=False)
def _load_horario(_v: int) -> list[dict]:
    return db.horario_listar()

@st.cache_data(show_spinner=False, ttl=300)
def _load_cache_scraping(fuente: str, _v: int) -> tuple[list, datetime | None]:
    datos, ts = db.cache_leer(fuente)
    return (datos or []), ts


def cargar_todo() -> dict:
    v = st.session_state.get("_data_v", 0)
    return {
        "tareas":  _load_tareas(v),
        "horario": _load_horario(v),
        "loyola":  _load_cache_scraping("loyola",  v)[0],
        "futbol":  _load_cache_scraping("sevilla", v)[0],
    }


# ─────────────────── LIMPIEZA DE PASADOS ───────────────────────────────────

def limpiar_pasados() -> int:
    """
    Elimina de la DB:
      - tareas COMPLETADAS con fecha (o fecha_fin) < hoy
      - eventos NO-rutina con fecha < hoy
    NO borra tareas pendientes pasadas: se muestran como atrasadas.
    """
    hoy = fstr(hoy_madrid())
    borradas = 0

    pre_t = db._exec(
        "SELECT COUNT(*) AS c FROM tareas WHERE estado='Completada' AND COALESCE(fecha_fin, fecha) < ?",
        [hoy]
    )
    pre_e = db._exec("SELECT COUNT(*) AS c FROM horario WHERE es_rutina = 0 AND fecha < ?", [hoy])
    borradas += int(pre_t[0]["c"]) if pre_t else 0
    borradas += int(pre_e[0]["c"]) if pre_e else 0

    if borradas:
        db._exec_write(
            "DELETE FROM tareas WHERE estado='Completada' AND COALESCE(fecha_fin, fecha) < ?",
            [hoy]
        )
        db._exec_write("DELETE FROM horario WHERE es_rutina = 0 AND fecha < ?", [hoy])
        bump_data()
    return borradas


# ─────────────────── ÍNDICE EVENTOS POR DÍA ────────────────────────────────

def construir_indice(data: dict, dia_min: date, dia_max: date) -> dict[str, list[dict]]:
    """
    Genera {fecha_iso: [items_ordenados]} para el rango [dia_min, dia_max].
    Filtra automáticamente fechas < hoy (no se muestran).
    """
    hoy_s = fstr(hoy_madrid())
    idx: dict[str, list[dict]] = defaultdict(list)

    # 1. Clases Loyola
    for c in data["loyola"]:
        f = c["fecha"]
        if f < hoy_s or not (fstr(dia_min) <= f <= fstr(dia_max)):
            continue
        h = c["hora"].split("-")[0].strip()
        idx[f].append({
            "tipo": "Clase", "titulo": c["asignatura"],
            "hora_sort": h, "hora": c["hora"],
            "subtitulo": f"📍 {c['aula']}", "icon": "🎓",
            "raw": {**c, "tipo": "Clase", "es_universidad": True},
        })

    # 2. Partidos Sevilla
    for p in data["futbol"]:
        f = p["fecha"]
        if f < hoy_s or not (fstr(dia_min) <= f <= fstr(dia_max)):
            continue
        h = p["hora"] or "00:00"
        idx[f].append({
            "tipo": "Futbol", "titulo": p["titulo"],
            "hora_sort": h, "hora": p["hora"] or "TBD",
            "subtitulo": p.get("aula", ""), "icon": "⚽",
            "raw": {**p, "tipo": "Futbol"},
        })

    # 3. Horario (rutinas + eventos únicos)
    rutinas, eventos_unicos = [], []
    for ev in data["horario"]:
        (rutinas if ev["es_rutina"] else eventos_unicos).append(ev)

    # 3a. Eventos únicos
    for ev in eventos_unicos:
        f = ev.get("fecha")
        if not f or f < hoy_s or not (fstr(dia_min) <= f <= fstr(dia_max)):
            continue
        idx[f].append({
            "tipo": "Evento", "titulo": ev["titulo"],
            "hora_sort": ev["hora_inicio"],
            "hora": f"{ev['hora_inicio']} - {ev['hora_fin']}",
            "subtitulo": ev.get("ubicacion", ""), "icon": "📅",
            "raw": {**ev},
        })

    # 3b. Rutinas: expandir solo en el rango pedido y >= hoy
    if rutinas:
        d = max(dia_min, hoy_madrid())
        while d <= dia_max:
            wd = d.weekday()
            f  = fstr(d)
            for ev in rutinas:
                if wd in ev["dias_semana"]:
                    idx[f].append({
                        "tipo": "Evento", "titulo": ev["titulo"],
                        "hora_sort": ev["hora_inicio"],
                        "hora": f"{ev['hora_inicio']} - {ev['hora_fin']}",
                        "subtitulo": ev.get("ubicacion", ""), "icon": "🔄",
                        "raw": {**ev},
                    })
            d += timedelta(days=1)

    # 4. Tareas (ya filtramos pasadas en limpiar_pasados, pero reforzamos)
    for t in data["tareas"]:
        if t["estado"] == "Completada":
            continue
        es_dl = bool(t.get("fecha_fin"))
        f     = t["fecha_fin"] if es_dl else t.get("fecha")
        if not f or f < hoy_s or not (fstr(dia_min) <= f <= fstr(dia_max)):
            continue
        hora_s = t.get("hora") or ("00:00" if t["dia_completo"] else "23:59")
        idx[f].append({
            "tipo": "Deadline" if es_dl else "Tarea",
            "titulo": t["titulo"],
            "hora_sort": hora_s, "hora": t.get("hora"),
            "subtitulo": t.get("prioridad", "Normal"),
            "icon": "⏰" if es_dl else "📝",
            "raw": {**t, "tipo": "tarea"},
        })

    # Ordenar
    for k in idx:
        idx[k].sort(key=lambda x: (x["hora_sort"] or "9999").replace(":", ""))
    return idx


# ─────────────────────────── DIALOG ────────────────────────────────────────

@st.dialog("Detalle")
def dialogo_detalle(item: dict):
    raw    = item["raw"]
    tipo   = raw.get("tipo", "")
    titulo = raw.get("titulo", raw.get("asignatura", "Sin título"))

    st.subheader(f"{item['icon']} {titulo}")
    st.caption(f"Tipo: {tipo}")
    st.divider()

    c1, c2 = st.columns(2)
    c1.markdown(f"**🕒 Hora:** {item['hora'] or 'Todo el día'}")
    if raw.get("aula"):      c1.markdown(f"**📍 Aula:** {raw['aula']}")
    if raw.get("ubicacion"): c1.markdown(f"**📍 Ubicación:** {raw['ubicacion']}")
    if raw.get("fecha"):     c2.markdown(f"**📅 Fecha:** {raw['fecha']}")
    if raw.get("fecha_fin"): c2.markdown(f"**⏰ Deadline:** {raw['fecha_fin']}")
    if raw.get("prioridad"): c2.markdown(f"**🚨 Prioridad:** {raw['prioridad']}")
    if raw.get("dias_semana"):
        ds = [DIAS_ABR[i] for i in raw["dias_semana"]]
        c2.markdown(f"**🔄 Días:** {', '.join(ds)}")

    st.divider()

    if tipo == "tarea":
        if raw.get("estado") != "Completada":
            if st.button("✅ Completar", use_container_width=True, type="primary"):
                db.tareas_actualizar(raw["id"], estado="Completada")
                bump_data(); notificar("exito", "Tarea completada ✅"); st.rerun()
        else:
            st.success("Ya completada")
    elif raw.get("es_universidad"):
        st.info("Evento del horario universitario oficial.")
    elif raw.get("id"):
        if st.button("🗑️ Eliminar evento", type="primary", use_container_width=True):
            db.horario_borrar(raw["id"])
            bump_data(); notificar("exito", "Evento eliminado 🗑️"); st.rerun()


# ─────────────────────────── VISTAS ────────────────────────────────────────

def vista_diaria(data: dict, fecha: date):
    hoy = hoy_madrid()
    if fecha < hoy:
        st.warning("Has seleccionado una fecha pasada — los eventos pasados no se muestran.")
        fecha = hoy

    idx = construir_indice(data, fecha, fecha)
    dia_items = idx.get(fstr(fecha), [])

    # ── Banner tareas atrasadas (pendientes con fecha < hoy) ──
    hoy_s = fstr(hoy)
    atrasadas = [
        t for t in data["tareas"]
        if t["estado"] != "Completada"
        and (t.get("fecha_fin") or t.get("fecha", "9999")) < hoy_s
    ]
    if atrasadas:
        st.error(f"🚨 {len(atrasadas)} tarea(s) atrasada(s)")
        with st.expander("Ver atrasadas"):
            for t in atrasadas:
                ref = t.get("fecha_fin") or t.get("fecha", "?")
                c1, c2, c3 = st.columns([5, 1, 1])
                c1.markdown(f"🔴 **{t['titulo']}** — {ref}")
                if c2.button("✅", key=f"atr_ok_{t['id']}", help="Completar"):
                    db.tareas_actualizar(t["id"], estado="Completada")
                    bump_data(); st.rerun()
                if c3.button("🗑️", key=f"atr_del_{t['id']}", help="Borrar"):
                    db.tareas_borrar(t["id"]); bump_data(); st.rerun()

    col_h, col_t = st.columns([1, 2])

    with col_h:
        st.subheader("🏫 Horario del día")
        eventos = [i for i in dia_items if i["tipo"] in ("Clase", "Futbol", "Evento")]
        if eventos:
            for ev in eventos:
                st.success(f"**{ev['hora']}**\n\n{ev['icon']} {ev['titulo']}\n\n{ev['subtitulo']}")
        else:
            st.info("Sin clases ni eventos hoy.")

    with col_t:
        st.subheader(f"📝 {fmt_dia_largo(fecha)}")
        tareas_dia = [i for i in dia_items if i["tipo"] in ("Tarea", "Deadline")]

        if not tareas_dia:
            st.info("✅ Sin tareas para este día.")
        else:
            for i in tareas_dia:
                raw = i["raw"]
                color = COLORES_TIPO.get(raw.get("tipo", "Otro"), "#888")
                hora_badge = (
                    f"<span style='background:#333;color:#fff;padding:2px 6px;"
                    f"border-radius:4px;font-size:.8em'>🕒 {raw['hora']}</span> "
                    if raw.get("hora") else ""
                )
                with st.container(border=True):
                    c1, c2 = st.columns([5, 1])
                    c1.markdown(
                        f"{hora_badge}<strong>{raw['titulo']}</strong> "
                        f"<span style='background:{color};color:#fff;padding:2px 6px;"
                        f"border-radius:4px;font-size:.8em'>{raw.get('tipo','')}</span>",
                        unsafe_allow_html=True
                    )
                    if c2.button("✅", key=f"d_ok_{raw['id']}"):
                        db.tareas_actualizar(raw["id"], estado="Completada")
                        bump_data(); st.rerun()

        st.divider()
        st.subheader("📋 Todas las pendientes")
        pendientes = sorted(
            [t for t in data["tareas"]
             if t["estado"] != "Completada"
             and (t.get("fecha_fin") or t.get("fecha", "9999")) >= fstr(hoy)],
            key=lambda t: (
                t.get("fecha_fin") or t.get("fecha", "9999-99-99"),
                PRIO_ORDEN.get(t.get("prioridad", "Normal"), 2)
            )
        )
        if not pendientes:
            st.success("🎉 ¡No tienes ninguna tarea pendiente!")
        else:
            for t in pendientes:
                _tarjeta_pendiente(t, hoy)


def _tarjeta_pendiente(t: dict, hoy: date):
    ref_str = t.get("fecha_fin") or t.get("fecha")
    prio    = t.get("prioridad", "Normal")
    color_p = PRIO_COLOR.get(prio, "#888")
    color_t = COLORES_TIPO.get(t.get("tipo", "Otro"), "#888")
    es_dl   = bool(t.get("fecha_fin"))

    try:
        dias = (date.fromisoformat(ref_str) - hoy).days
        if dias == 0:   dias_txt, dias_color = "HOY", "#FF4B4B"
        elif dias == 1: dias_txt, dias_color = "mañana", "#FFA500"
        elif dias <= 3: dias_txt, dias_color = f"{dias}d", "#FFA500"
        elif dias <= 7: dias_txt, dias_color = f"{dias}d", "#1E90FF"
        else:           dias_txt, dias_color = f"{dias}d", "#555"
    except Exception:
        dias_txt, dias_color = ref_str or "sin fecha", "#555"

    icono = "⏰" if es_dl else "📅"
    with st.container(border=True):
        c1, c2 = st.columns([5, 1])
        c1.markdown(
            f"<div style='display:flex;align-items:center;gap:6px;flex-wrap:wrap'>"
            f"<span style='background:{dias_color};color:white;padding:2px 8px;"
            f"border-radius:12px;font-size:.8em;font-weight:bold'>{icono} {dias_txt}</span>"
            f"<strong>{t['titulo']}</strong>"
            f"<span style='background:{color_t};color:white;padding:2px 6px;"
            f"border-radius:4px;font-size:.75em'>{t.get('tipo','')}</span>"
            f"<span style='background:{color_p};color:white;padding:2px 6px;"
            f"border-radius:4px;font-size:.75em'>{prio}</span>"
            f"</div>"
            f"<div style='font-size:.8em;color:#888;margin-top:2px'>"
            f"{'Deadline' if es_dl else 'Fecha'}: {ref_str or '—'}</div>",
            unsafe_allow_html=True
        )
        if c2.button("✅", key=f"p_ok_{t['id']}"):
            db.tareas_actualizar(t["id"], estado="Completada")
            bump_data(); st.rerun()


def _grid_css():
    st.markdown("""<style>
        @media (max-width:900px) {
            div[data-testid="stHorizontalBlock"] {
                flex-direction:row!important; flex-wrap:nowrap!important; gap:0!important;
            }
            div[data-testid="column"] { flex:1 1 0!important; min-width:0!important; padding:0!important; }
        }
        @media (orientation:portrait) and (max-width:600px) {
            div[data-testid="stHorizontalBlock"] {
                display:grid!important; grid-template-columns:repeat(7,1fr)!important; gap:1px!important;
            }
            div[data-testid="column"] { width:auto!important; min-width:0!important; padding:0!important; }
            div[data-testid="column"] p { font-size:3vw!important; text-align:center!important; margin:0!important; }
            div[data-testid="stButton"] button { font-size:4vw!important; padding:0!important;
                min-height:25px!important; border:none!important; background:transparent!important; }
            div[data-testid="stButton"] button p { max-width:1.5em!important; overflow:hidden!important; margin:0 auto!important; }
        }
    </style>""", unsafe_allow_html=True)


def vista_semanal(data: dict, fecha: date):
    _grid_css()
    st.subheader("Vista Semanal")
    lunes = fecha - timedelta(days=fecha.weekday())
    domingo = lunes + timedelta(days=6)
    idx = construir_indice(data, lunes, domingo)
    hoy = hoy_madrid()

    cols = st.columns(7)
    for i, col in enumerate(cols):
        dia = lunes + timedelta(days=i)
        es_pasado = dia < hoy
        if   dia == fecha: bg, border, tc = "#1E90FF", "3px solid #1E90FF", "white"
        elif dia == hoy:   bg, border, tc = "#FF4B4B", "3px solid #FF4B4B", "white"
        else:              bg, border, tc = "transparent", "1px solid #444", "var(--text-color)"

        with col:
            st.markdown(
                f"<div style='text-align:center;border-bottom:{border};margin-bottom:5px;"
                f"opacity:{0.4 if es_pasado else 1}'>"
                f"<div style='background:{bg};color:{tc};border-radius:4px 4px 0 0;padding:2px'>"
                f"<strong>{DIAS_ABR[i]}</strong></div>"
                f"<div style='font-size:1.2em;padding:5px'>{dia.day}</div></div>",
                unsafe_allow_html=True
            )
            if es_pasado:
                continue
            for item in idx.get(fstr(dia), []):
                trunc = (item["titulo"][:10] + "..") if len(item["titulo"]) > 10 else item["titulo"]
                if st.button(f"{item['icon']} {item['hora_sort']} {trunc}",
                             key=f"sw_{i}_{item['hora_sort']}_{item['titulo'][:6]}",
                             use_container_width=True):
                    dialogo_detalle(item)


def vista_mensual(data: dict, fecha: date):
    _grid_css()
    st.subheader(f"Vista Mensual — {MESES_ES[fecha.month-1]} {fecha.year}")
    calendar.setfirstweekday(calendar.MONDAY)
    cal = calendar.monthcalendar(fecha.year, fecha.month)

    primer = date(fecha.year, fecha.month, 1)
    ultimo = date(fecha.year, fecha.month,
                  calendar.monthrange(fecha.year, fecha.month)[1])
    idx = construir_indice(data, primer, ultimo)
    hoy = hoy_madrid()

    for semana in cal:
        cols = st.columns(7)
        for i, day_num in enumerate(semana):
            with cols[i]:
                if day_num == 0:
                    st.markdown("<div style='min-height:80px'></div>", unsafe_allow_html=True)
                    continue
                dia = date(fecha.year, fecha.month, day_num)
                es_pasado = dia < hoy
                color_num = "#FF4B4B" if dia == hoy else "#1E90FF" if dia == fecha else "#AAA"
                border    = f"2px solid {color_num}" if dia in (hoy, fecha) else "1px solid #444"
                opac      = 0.4 if es_pasado else 1

                st.markdown(
                    f"<div style='text-align:right;font-weight:bold;border-bottom:{border};"
                    f"margin-bottom:4px;color:{color_num};opacity:{opac}'>{day_num} "
                    f"<span style='font-size:.75em;opacity:.7;font-weight:normal'>{DIAS_ABR[i]}</span></div>",
                    unsafe_allow_html=True
                )
                if es_pasado:
                    continue
                for item in idx.get(fstr(dia), []):
                    trunc = (item["titulo"][:8] + "..") if len(item["titulo"]) > 8 else item["titulo"]
                    if st.button(f"{item['icon']} {trunc}",
                                 key=f"sm_{day_num}_{i}_{item['hora_sort']}_{item['titulo'][:6]}",
                                 use_container_width=True,
                                 help=f"{item['hora_sort']} — {item['titulo']}"):
                        dialogo_detalle(item)


# ─────────────────────── FORMULARIOS ───────────────────────────────────────

def vista_nueva_tarea():
    st.subheader("➕ Nueva Tarea")
    with st.form("nueva_tarea", clear_on_submit=True):
        c_conf, c_form = st.columns([1, 3])
        with c_conf:
            modo = st.radio("Modo", ["📅 Día concreto", "⏰ Deadline"])
        with c_form:
            titulo = st.text_input("Título *")
            c1, c2 = st.columns(2)
            if "Deadline" in modo:
                f_fin = c1.date_input("Fecha límite", hoy_madrid(), min_value=hoy_madrid())
                f_ini = None
            else:
                f_ini = c1.date_input("Fecha", hoy_madrid(), min_value=hoy_madrid())
                f_fin = None
            dia_completo = c2.checkbox("Todo el día", value=True)
            hora = c2.time_input("Hora", time(10, 0), step=900) if not dia_completo else None
            prio = c1.selectbox("Prioridad", ["Normal", "Importante", "Urgente"])
            tipo = c2.selectbox("Tipo", TIPOS_TAREA)

            if st.form_submit_button("💾 Guardar", type="primary", use_container_width=True):
                if not titulo.strip():
                    st.error("El título es obligatorio."); return
                db.tareas_crear(
                    titulo=titulo.strip(), tipo=tipo, prioridad=prio,
                    fecha=fstr(f_ini) if f_ini else fstr(hoy_madrid()),
                    fecha_fin=fstr(f_fin) if f_fin else None,
                    hora=hora.strftime("%H:%M") if hora else None,
                    dia_completo=dia_completo,
                )
                bump_data(); notificar("exito", "✅ Tarea guardada"); st.rerun()


def vista_nuevo_evento():
    st.subheader("➕ Nuevo Evento / Horario")
    with st.form("nuevo_evento", clear_on_submit=True):
        c_conf, c_form = st.columns([1, 3])
        with c_conf:
            tipo_entrada = st.radio("Tipo", ["🔄 Rutina Semanal", "📅 Evento Único"])
            st.caption("Rutina: se repite cada semana. Evento: día concreto.")
        with c_form:
            titulo    = st.text_input("Título / Asignatura")
            ubicacion = st.text_input("Ubicación / Aula")
            dias_sel, fecha_ev = [], None
            if "Rutina" in tipo_entrada:
                st.write("Días de la semana:")
                cols_d = st.columns(7)
                for idx, col in enumerate(cols_d):
                    if col.checkbox(DIAS_ABR[idx], key=f"chk2_dia_{idx}"):
                        dias_sel.append(idx)
            else:
                fecha_ev = st.date_input("Fecha", hoy_madrid(), min_value=hoy_madrid())
            c1, c2 = st.columns(2)
            h_ini = c1.time_input("Inicio", time(10, 0))
            h_fin = c2.time_input("Fin",    time(11, 0))
            if st.form_submit_button("💾 Guardar", type="primary", use_container_width=True):
                if not titulo.strip():
                    st.error("El título es obligatorio."); return
                es_rutina = "Rutina" in tipo_entrada
                if es_rutina and not dias_sel:
                    st.error("Selecciona al menos un día."); return
                db.horario_crear(
                    titulo=titulo.strip(), ubicacion=ubicacion,
                    es_rutina=es_rutina, dias_semana=dias_sel,
                    fecha=fstr(fecha_ev) if fecha_ev else None,
                    hora_inicio=h_ini.strftime("%H:%M"),
                    hora_fin=h_fin.strftime("%H:%M"),
                )
                bump_data(); notificar("exito", "✅ Evento guardado"); st.rerun()


# ─────────────────── GESTIÓN GLOBAL ────────────────────────────────────────

def vista_gestionar(data: dict):
    st.subheader("📋 Gestión Global")
    tab_t, tab_h = st.tabs(["📝 Tareas", "📅 Horarios"])
    hoy_s = fstr(hoy_madrid())

    with tab_t:
        # Solo no-pasadas; las pasadas ya fueron eliminadas al iniciar.
        pendientes = sorted(
            [t for t in data["tareas"] if t["estado"] != "Completada"],
            key=lambda t: (PRIO_ORDEN.get(t["prioridad"], 3),
                           t.get("fecha_fin") or t.get("fecha", "9999"))
        )
        completadas = [t for t in data["tareas"] if t["estado"] == "Completada"]

        st.markdown(f"**Pendientes: {len(pendientes)}**")
        for t in pendientes:
            _tarjeta_tarea_gestion(t)
        st.divider()
        with st.expander(f"Completadas ({len(completadas)})"):
            for t in completadas:
                _tarjeta_tarea_gestion(t)

    with tab_h:
        horario = data["horario"]
        if not horario:
            st.info("No hay horarios ni eventos creados.")
            return
        eventos = sorted([h for h in horario if not h["es_rutina"]], key=lambda x: x.get("fecha",""))
        rutinas = [h for h in horario if h["es_rutina"]]
        for ev in eventos + rutinas:
            _tarjeta_evento_gestion(ev)


def _tarjeta_tarea_gestion(t: dict):
    color_prio = {"Urgente": "red", "Importante": "orange"}.get(t["prioridad"], "green")
    opac = "0.5" if t["estado"] == "Completada" else "1"
    icon = "✅" if t["estado"] == "Completada" else "⬜"
    ref  = t.get("fecha_fin") or t.get("fecha", "?")
    hora_str = f" @ {t['hora']}" if not t["dia_completo"] and t.get("hora") else ""

    with st.container(border=True):
        c_info, c_btn = st.columns([5, 2])
        c_info.markdown(
            f"<h4 style='margin:0;opacity:{opac}'>{icon} {t['titulo']}</h4>"
            f"<span style='color:{color_prio};font-weight:bold'>{t['prioridad']}</span>"
            f" | {t['tipo']} | <strong>{ref}{hora_str}</strong>",
            unsafe_allow_html=True
        )
        with c_btn:
            ca1, ca2, ca3 = st.columns(3)
            if t["estado"] != "Completada":
                if ca1.button("✅", key=f"g_ok_{t['id']}", help="Completar"):
                    db.tareas_actualizar(t["id"], estado="Completada"); bump_data(); st.rerun()
            else:
                if ca1.button("↩️", key=f"g_un_{t['id']}", help="Deshacer"):
                    db.tareas_actualizar(t["id"], estado="Pendiente"); bump_data(); st.rerun()

            with ca2.popover("✏️"):
                with st.form(f"edit_t_{t['id']}"):
                    e_tit = st.text_input("Título", t["titulo"])
                    es_dl = bool(t.get("fecha_fin"))
                    base_fecha = t.get("fecha_fin") or t.get("fecha") or fstr(hoy_madrid())
                    e_fecha = st.date_input(
                        "Deadline" if es_dl else "Fecha",
                        datetime.strptime(base_fecha, "%Y-%m-%d").date(),
                        min_value=hoy_madrid()
                    )
                    e_all  = st.checkbox("Todo el día", value=bool(t.get("dia_completo", True)))
                    e_hora = None
                    if not e_all:
                        e_hora = st.time_input("Hora",
                            datetime.strptime(t.get("hora") or "09:00", "%H:%M").time())
                    e_estado = st.selectbox("Estado", ["Pendiente", "Completada"],
                                            index=0 if t["estado"] == "Pendiente" else 1)
                    e_prio   = st.selectbox("Prioridad", ["Normal","Importante","Urgente"],
                                            index=["Normal","Importante","Urgente"].index(t["prioridad"]))
                    if st.form_submit_button("Guardar"):
                        campos = dict(titulo=e_tit, estado=e_estado, prioridad=e_prio,
                                      dia_completo=e_all,
                                      hora=e_hora.strftime("%H:%M") if e_hora else None)
                        if es_dl: campos["fecha_fin"] = fstr(e_fecha)
                        else:     campos["fecha"]     = fstr(e_fecha)
                        db.tareas_actualizar(t["id"], **campos)
                        bump_data(); notificar("exito", "✏️ Tarea actualizada"); st.rerun()

            if ca3.button("🗑️", key=f"g_del_{t['id']}", help="Borrar"):
                db.tareas_borrar(t["id"]); bump_data()
                notificar("exito", "🗑️ Tarea eliminada"); st.rerun()


def _tarjeta_evento_gestion(ev: dict):
    icon = "🔄" if ev["es_rutina"] else "📅"
    dias_txt = ", ".join(DIAS_ABR[i] for i in ev["dias_semana"]) if ev["es_rutina"] else ev.get("fecha","?")
    with st.container(border=True):
        c1, c2 = st.columns([5, 2])
        c1.markdown(f"**{icon} {ev['titulo']}** ({ev['hora_inicio']} - {ev['hora_fin']})")
        c1.caption(f"{ev.get('ubicacion','')} | {dias_txt}")
        with c2:
            ca_e, ca_d = st.columns(2)
            with ca_e.popover("✏️"):
                with st.form(f"edit_ev_{ev['id']}"):
                    e_tit = st.text_input("Título", ev["titulo"])
                    e_ubi = st.text_input("Ubicación", ev.get("ubicacion",""))
                    e_rut = st.toggle("Rutina semanal", ev["es_rutina"])
                    e_dias, e_fev = [], None
                    if e_rut:
                        sel = st.multiselect("Días", DIAS_ABR,
                                             default=[DIAS_ABR[i] for i in ev["dias_semana"]])
                        e_dias = [DIAS_ABR.index(d) for d in sel]
                    else:
                        try:
                            def_f = datetime.strptime(ev.get("fecha", fstr(hoy_madrid())), "%Y-%m-%d").date()
                        except Exception:
                            def_f = hoy_madrid()
                        e_fev = st.date_input("Fecha", def_f, min_value=hoy_madrid())
                    try: t_i = datetime.strptime(ev.get("hora_inicio","09:00"), "%H:%M").time()
                    except: t_i = time(9,0)
                    try: t_f = datetime.strptime(ev.get("hora_fin","10:00"), "%H:%M").time()
                    except: t_f = time(10,0)
                    e_hi = st.time_input("Inicio", t_i)
                    e_hf = st.time_input("Fin",    t_f)
                    if st.form_submit_button("Guardar"):
                        db.horario_actualizar(ev["id"],
                            titulo=e_tit, ubicacion=e_ubi, es_rutina=e_rut,
                            dias_semana=e_dias,
                            fecha=fstr(e_fev) if e_fev else None,
                            hora_inicio=e_hi.strftime("%H:%M"),
                            hora_fin=e_hf.strftime("%H:%M"))
                        bump_data(); notificar("exito", "✏️ Evento actualizado"); st.rerun()
            if ca_d.button("🗑️", key=f"g_dev_del_{ev['id']}"):
                db.horario_borrar(ev["id"]); bump_data()
                notificar("exito", "🗑️ Evento eliminado"); st.rerun()


# ─────────────────────── SCRAPING (sidebar) ────────────────────────────────

def actualizar_scraping(fuente: str, force: bool = False):
    if not force:
        _, ts = db.cache_leer(fuente)
        if ts and (ahora_madrid().replace(tzinfo=None) - ts).total_seconds() < CACHE_HORAS_SCRAPING * 3600:
            return
    try:
        datos = scraper.scrape_loyola() if fuente == "loyola" else scraper.scrape_sevilla()
        db.cache_guardar(fuente, datos); bump_data()
        notificar("exito", f"✅ {fuente.capitalize()} actualizado ({len(datos)} entradas)")
    except Exception as e:
        notificar("error", f"❌ Error actualizando {fuente}: {e}")


# ─────────────────────────── MAIN ──────────────────────────────────────────

def main():
    db.init_db()
    st.session_state.setdefault("_data_v", 0)

    mostrar_notificacion()

    # Limpieza única por sesión/día
    last_clean = st.session_state.get("_last_clean")
    hoy_s = fstr(hoy_madrid())
    if last_clean != hoy_s:
        borradas = limpiar_pasados()
        st.session_state["_last_clean"] = hoy_s
        if borradas:
            st.toast(f"🧹 {borradas} elemento(s) pasado(s) eliminado(s)")

    data = cargar_todo()

    # ── Sidebar ──
    with st.sidebar:
        st.title("🎓 AutoGestor v2")
        vista = st.radio(
            "Vista:",
            ["Diaria", "Semanal", "Mensual", "---",
             "➕ Nueva Tarea", "➕ Nuevo Evento", "📋 Gestionar Todo"],
            index=0, label_visibility="collapsed",
        )
        st.divider()

        st.header("📅 Fecha")
        fecha = st.date_input("Base", hoy_madrid(), min_value=hoy_madrid())
        st.info(f"📍 {fmt_dia_corto(fecha)}")
        st.divider()

        st.header("🔄 Scraping")
        if st.button("Actualizar Loyola", use_container_width=True):
            with st.spinner("Scrapeando Loyola..."):
                actualizar_scraping("loyola", force=True)
            st.rerun()
        if st.button("Actualizar Sevilla FC", use_container_width=True):
            with st.spinner("Scrapeando Sevilla FC..."):
                actualizar_scraping("sevilla", force=True)
            st.rerun()

        ts_l = _load_cache_scraping("loyola",  st.session_state["_data_v"])[1]
        ts_s = _load_cache_scraping("sevilla", st.session_state["_data_v"])[1]
        if ts_l: st.caption(f"Loyola: {ts_l.strftime('%d/%m %H:%M')}")
        if ts_s: st.caption(f"Sevilla: {ts_s.strftime('%d/%m %H:%M')}")

    # ── Router ──
    st.title("Diario" if vista == "Diaria" else vista)

    if   vista == "Diaria":           vista_diaria(data, fecha)
    elif vista == "Semanal":          vista_semanal(data, fecha)
    elif vista == "Mensual":          vista_mensual(data, fecha)
    elif vista == "➕ Nueva Tarea":   vista_nueva_tarea()
    elif vista == "➕ Nuevo Evento":  vista_nuevo_evento()
    elif vista == "📋 Gestionar Todo": vista_gestionar(data)


if __name__ == "__main__":
    main()
